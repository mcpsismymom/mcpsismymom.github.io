...
Engine.prototype = {
			...
			case 0:
				...
				if(_gthis.control.frame + 1 >= player.video.pauseFrame) {
					if(_gthis.fullgameVideo == null) {
						...
						console.log("tas_haxe_files/Engine.hx:105:","[PAUSE ] @ " + (_gthis.control.frame + 1));
						_gthis.control.silent = false;
					} else if(_gthis.fullgameLevelCounter >= 1 && _gthis.fullgameLevelCounter <= 25) {
	...
	,primeControls: function(buffer) {
		...
		if(this.initialDirection == 1) {
			if(buffer) {
				console.log("tas_haxe_files/Engine.hx:208:","---> Holding controls: LEFT.");
			}
			this.sendGameInput(37,true);
		}
		if(this.initialDirection == 2) {
			if(buffer) {
				console.log("tas_haxe_files/Engine.hx:213:","---> Holding controls: RIGHT.");
			}
			this.sendGameInput(39,true);
		}
		if(this.initialDirection == 0) {
			if(buffer) {
				console.log("tas_haxe_files/Engine.hx:218:","---> Holding controls: NONE.");
			}
		}
	}
	,resetLevel: function(slot,replay) {
		...
		console.log("tas_haxe_files/Engine.hx:225:","[" + (replay ? "REPLAY" : "RESET to") + " " + (slot == null ? "start" : "slot " + (slot == null ? "null" : "" + slot) + "...") + "]");
	...
	,handleInterfaceInput: function(input,ctrlKey,altKey) {
		...
		if(input == CoffeeInput.StepFrame && this.control.paused) {
			console.log("tas_haxe_files/Engine.hx:246:","[STEP  ] @ " + (this.control.frame + 1));
			this.triggerPausedCallback();
			return true;
		}
		if(input == CoffeeInput.Pause) {
			if(!oldControl.paused) {
				console.log("tas_haxe_files/Engine.hx:254:","[PAUSE ] @ " + (this.control.frame + 1));
			}
			this.control.pause();
			return true;
		}
		...
		if(playAction) {
			...
			if(oldControl.paused) {
				console.log("tas_haxe_files/Engine.hx:275:","[PLAY  ] @ " + this.control.frame);
			}
			this.triggerPausedCallback();
			return true;
		}
		...
		if(input._hx_index == 7) {
			...
			if(ctrlKey && !altKey) {
				...
				console.log("tas_haxe_files/Engine.hx:318:","[SAVE slot " + slot + "] @ " + this.control.frame);
				console.log("tas_haxe_files/Engine.hx:319:","data: " + video.toString());
	...
	,onScene: function(action,levelNum) {
		console.log("tas_haxe_files/Engine.hx:330:","[SCENE " + levelNum + "]");
...
VideoRecorder.prototype = {
	recordKey: function(frame,keyCode,down,silent) {
		...
		case 0:
			...
			if(!silent) {
				console.log("tas_haxe_files/Video.hx:127:","---> " + Video.showActionCode(action) + " " + (down ? "down" : "up  ") + " @ " + frame);
			}
			break;
		case 1:
			return;
